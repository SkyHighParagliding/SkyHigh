...
router.put("/:id", requireAuth, async (req, res) => {
  ...
  try {
      const update = await db.prepare(`
        UPDATE sites SET
          name = @name, type = @type,
          pgRating = CASE WHEN @pgRating IS NOT NULL AND @pgRating::text != '' THEN @pgRating ELSE pgRating END,
          hgRating = CASE WHEN @hgRating IS NOT NULL AND @hgRating::text != '' THEN @hgRating ELSE hgRating END,
          ...
          landing = @landing, hazards = @hazards, rules = @rules, image = CASE WHEN @image IS NOT NULL AND @image::text != '' THEN @image ELSE image END,
          ...
          siteContactPhone = CASE WHEN @siteContactPhone IS NOT NULL AND @siteContactPhone::text != '' THEN @siteContactPhone ELSE siteContactPhone END,
          navigateTo = CASE WHEN @navigateTo IS NOT NULL AND @navigateTo::text != '' THEN @navigateTo ELSE navigateTo END,
          launchHeight = CASE WHEN @launchHeight IS NOT NULL AND @launchHeight::text != '' THEN @launchHeight ELSE launchHeight END,
          launchHeightHigh = CASE WHEN @launchHeightHigh IS NOT NULL AND @launchHeightHigh::text != '' THEN @launchHeightHigh ELSE launchHeightHigh END,
          launchHeight2 = CASE WHEN @launchHeight2 IS NOT NULL AND @launchHeight2::text != '' THEN @launchHeight2 ELSE launchHeight2 END,
          landingHeight2 = CASE WHEN @landingHeight2 IS NOT NULL AND @landingHeight2::text != '' THEN @landingHeight2 ELSE landingHeight2 END,
          hoodedPloversLink = CASE WHEN @hoodedPloversLink IS NOT NULL AND @hoodedPloversLink::text != '' THEN @hoodedPloversLink ELSE hoodedPloversLink END,
          hoodedPloversActive = @hoodedPloversActive,
          emergencyMarker = CASE WHEN @emergencyMarker IS NOT NULL AND @emergencyMarker::text != '' THEN @emergencyMarker ELSE emergencyMarker END,
          what3words = CASE WHEN @what3words IS NOT NULL AND @what3words::text != '' THEN @what3words ELSE what3words END,
          weatherStationLink = CASE WHEN @weatherStationLink IS NOT NULL AND @weatherStationLink::text != '' THEN @weatherStationLink ELSE weatherStationLink END,
          isSkyHighSite = @isSkyHighSite, crossLeft = @crossLeft, crossRight = @crossRight, overrideHideClosed = @overrideHideClosed,
          unassignedText = CASE WHEN @unassignedText IS NOT NULL AND @unassignedText::text != '' THEN @unassignedText ELSE unassignedText END,
          essentialInfoImages = CASE WHEN @essentialInfoImages IS NOT NULL AND @essentialInfoImages::text != '' AND @essentialInfoImages::text != '[]' THEN @essentialInfoImages ELSE essentialInfoImages END,
          essentialInfoText = CASE WHEN @essentialInfoText IS NOT NULL AND @essentialInfoText::text != '' THEN @essentialInfoText ELSE essentialInfoText END,
          siteguideVersion = CASE WHEN @siteguideVersion IS NOT NULL AND @siteguideVersion::text != '' THEN @siteguideVersion ELSE siteguideVersion END,
          siteguideScrapedAt = CASE WHEN @siteguideScrapedAt IS NOT NULL AND @siteguideScrapedAt::text != '' THEN @siteguideScrapedAt ELSE siteguideScrapedAt END,
