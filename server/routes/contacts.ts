...
router.get("/public/committee", asyncHandler(async (req, res) => {
  const members = await db.prepare(
    "SELECT id, name, surname, organisation, phone, email, position, isSafetyCommittee, showTelegram, showPhone, showEmail, showAdminEmail FROM contacts WHERE isCommittee = 1 AND displayCommittee = 1 ORDER BY name ASC"
  ).all() as { id: string; name: string; surname: string; organisation: string; phone: string; email: string; position: string | null; isSafetyCommittee?: number; showTelegram: number; showPhone: number; showEmail: number; showAdminEmail: number }[];
...
router.get("/search", requireAuth, asyncHandler(async (req, res) => {
  ...
  const contacts = await db.prepare(
    "SELECT id, organisation, name, surname, phone, email, notes, position, isAdmin, isCommittee, isContractor, isParksVic, isSafetyCommittee, isSocialMedia, soAuthorised, displayCommittee, displaySafety, showTelegram, showPhone, showEmail, showAdminEmail, photoAuthorised, createdAt, updatedAt FROM contacts WHERE name LIKE ? OR surname LIKE ? OR organisation LIKE ? ORDER BY organisation ASC, name ASC LIMIT ? OFFSET ?"
...
router.get("/:id", requireAuth, asyncHandler(async (req, res) => {
  const contact = await db.prepare("SELECT id, organisation, name, surname, phone, email, notes, isAdmin, isCommittee, isContractor, isParksVic, isSafetyCommittee, isSocialMedia, soAuthorised, displayCommittee, displaySafety, showTelegram, showPhone, showEmail, showAdminEmail, photoAuthorised, createdAt, updatedAt FROM contacts WHERE id = ?").get(req.params.id);
...
router.post("/", requireAuth, asyncHandler(async (req, res) => {
  ...
  const contact = await db.prepare("SELECT id, organisation, name, surname, phone, email, notes, isAdmin, isCommittee, isContractor, isParksVic, isSafetyCommittee, isSocialMedia, soAuthorised, displayCommittee, displaySafety, showTelegram, showPhone, showEmail, showAdminEmail, photoAuthorised, createdAt, updatedAt FROM contacts WHERE id = ?").get(id);
...
// TODO: photoUrl column doesn't exist on production database
