    ...
    items: [
      ...
      },
      {
        ...
        details: [
          ...
          "VITE_TERRAIN_TILE_URL — Override the terrain tile base URL used by the client-side Ground elevation sampler (default: https://s3.amazonaws.com/elevation-tiles-prod/terrarium). Useful if self-hosting terrain tiles or switching tile providers. Must be set at build time (Vite env var).",
        ],
      },
    ],
  },
    ...
    items: [
      ...
      },
      {
        ...
        details: [
          ...
          "GET /elevation-at — Fallback terrain elevation lookup by lat/lon (used by elevationPoint.ts when the browser tile cache misses; returns metres AMSL)",
    ...
    items: [
      ...
      },
      {
        ...
        details: [
          ...
          "server/grid/ — Multi-source gridded forecast layer. Providers are tiered (Open-Meteo API → Open-Meteo S3 ECMWF → S3 GFS → NOAA NOMADS) and the orchestrator gap-fills each tier from the next, tracking per-point provenance and warning when model families are mixed. fineGrid.ts (0.15° wind + weather) and thermalGrid.ts (0.09° CAPE/boundary-layer) share one pipeline; extract.ts turns the stored grid into overlay, particle and per-site forecast payloads. Stores the full 36-hour range for days 1-2.",
          ...
          "src/components/windmap/terrainTiles.ts — Client-side terrain tile sampler. Fetches AWS Open Data 'terrarium' tiles at zoom level 12 (~30 m/px resolution at Victorian latitudes) and decodes RGB pixel values to metres AMSL using bilinear interpolation. Maintains an in-memory LRU cache of 64 tiles. Sampling a resident tile resolves in about 0.2 ms (measured), against a ~465 ms round-trip for the server fallback.",
          "src/components/windmap/elevationPoint.ts — Two-tier elevation facade. Tries the local terrain tile cache first; falls back to GET /api/weather/elevation-at only when the needed tile is not yet resident. On fallback it also kicks off a background tile fetch so the next tap on the same area is instant.",
          "Terrain tile prefetch: both wind and thermal map canvases prefetch the 3×3 block of z12 tiles around the map centre, debounced 300 ms after each pan or zoom. A 3×3 block covers roughly 29 × 22 km and amounts to about 180 KB. The server route GET /api/weather/elevation-at is retained as the fallback and is unchanged.",
          "Terrain tile source: defaults to https://s3.amazonaws.com/elevation-tiles-prod/terrarium. Overridable via VITE_TERRAIN_TILE_URL environment variable.",
        ],
      },
      {
        icon: <Wrench className="w-5 h-5" />,
