  ...
  const sections = [
    ...
    },
    {
      ...
      steps: [
        ...
        "Wind Grid (5:00 am daily): Fetches 2-day hourly wind speed, gusts, direction, temperature, weather code, precipitation, cloud cover, visibility, CAPE, and boundary layer height at 0.15° resolution (~17km). Powers the wind speed colour heatmap and particle animation on the wind map. About 7 tiles × ~3 seconds apart — takes roughly 30 seconds to complete.",
        ...
        "Live Progress: While a fetch is running, the status panel updates every 2 seconds: e.g. 'Wind Grid — Tile 5/7 · 71% · 5000/6912 pts · ok — 0:18'. If a tile fails, it shows in red. When the fetch completes, the result is shown for 5 seconds before the panel clears.",
        ...
        "Troubleshooting — No colour on wind map / all grey: The Wind Grid fetch likely failed. Check the 'Wind Grid' last run row — if it shows an error, click 'Wind Grid → Fetch Now'. Watch the progress panel to confirm tiles are completing.",
        "Troubleshooting — Thermal overlay missing: Check the 'Thermal Grid' last run row. If it shows 'partial — N tiles, retry X/4 in Ymin', the auto-retry is still running — wait for it to finish. If it shows 'unresolved after 4 retries', click 'Thermal Grid → Fetch Now' to manually retry.",
        ...
        "Troubleshooting — Tide chart flat line: The Wind Grid data is stale (the site forecast relies on the wind grid to position the tide window). Click 'Wind Grid → Fetch Now'. The tide chart recovers automatically after the next live weather scraper cycle (15–30 min)."
      ]
    },
    {
