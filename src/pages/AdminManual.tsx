  ...
  const sections = [
    ...
    },
    {
      ...
      steps: [
        ...
        "Three Wind Grids: The wind map is powered by three independent ECMWF IFS grids fetched daily. Each covers the same geographic area (Tasmania to Canberra, eastern SA to NSW coast) and runs on its own schedule. All three buttons are in the 'Grid Data' card.",
        "Wind (5:00 am daily): Fetches 2-day hourly wind speed, gusts, direction, temperature, weather code, precipitation, cloud cover, visibility, CAPE, and boundary layer height at 0.15° resolution (~17km). Powers the wind speed colour heatmap and particle animation on the wind map. About 7 tiles × ~3 seconds apart — takes roughly 30 seconds to complete.",
        "Thermal (5:26 am daily): Fetches CAPE, boundary-layer height, low cloud cover, and the stability data (lifted index / CIN) at 0.09° resolution (~10km) that powers the thermal overlay. The map colours thermal strength by W* (updraft velocity) derived from these; the cloud and stability fields drive the cumulus glyphs, overcast sheet, and overdevelopment warning (see Wind Map below). Takes 1–2 minutes for a full fetch. If some tiles fail, they are automatically retried up to 4 times at increasing intervals (+30 min, +60 min, +120 min, +240 min) — no action needed. The Last Run row shows the current retry status.",
        "7-Day (5:30 am daily): Fetches 8 days of weather at 0.5° resolution, keeps days 3–8 at four time slots per day (7am, 11am, 3pm, 7pm Melbourne time). Powers the 7-day outlook cards on each site's detail page. The fetch time is a configurable setting (schedExtendedForecastHour/Minute), not a fixed cron.",
