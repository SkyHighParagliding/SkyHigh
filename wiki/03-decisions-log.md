...
## DECISION-009: Automated 1Password Lifecycle Integration (.env draw/wipe)
...
### Context
Committing raw API keys and passwords to version control is a major security risk. Standard local `.env` files are easily leaked if committed accidentally. Standard `.env` bypass approaches (like raw manual copy-pasting of keys or manual `op signin` session configurations) introduce high developer friction and break when servers restart or when users switch context. We need an automated, friction-free way to keep credentials stored securely in 1Password, drawing them to a local, gitignored `.env` file at session start and securely erasing them upon session end.

### Options Considered
1. **Manual copy-pasting of `.env` files** — Keeps `.env` out of Git but exposes it on disk permanently. High developer friction when credentials change.
2. **`direnv` with `op read` (`.envrc`)** — Outstanding security, variables loaded into shell memory. However, doesn't natively map to Vite/Node configurations that expect `.env` files on disk, or to development servers launched directly by non-terminal tools.
3. **Automated `.env` draw/wipe lifecycle** (chosen) — A "Zero-Config" setup where a `.env.template` with standard 1Password references (`{{ op://... }}`) resides in Git. Custom scripts draw secrets directly into a gitignored `.env` at shell startup/directory change and wipe them on exit.

### Chosen
**Automated `.env` draw/wipe lifecycle**: Centralized scripts `draw-env.ps1` and `wipe-env.ps1` handle the lifecycle:
- Draws secrets using `op inject` automatically on shell startup or agent start (`SessionStart` hook).
- Automatically draws when navigating (`cd`) into a folder with a `.env.template` using the PowerShell `$PROFILE` `LocationChangedAction` callback.
- Erases the `.env` on directory exit (`cd` out), shell exit, or agent shutdown (`SessionEnd` hook).

### Rationale
- **Frictionless:** Secrets are fetched on demand. You simply unlock 1Password Desktop, and you're ready to code.
- **Zero Raw Secrets on Disk:** Secrets are automatically drawn on demand and securely deleted on exit.
- **Vite/Node Compatibility:** Emits standard `.env` variables so Vite and Express find them natively, without requiring custom launcher commands or complex wrappers.
- **Safety First:** Gitignores are fully verified to ensure `.env` can never be committed.

### Implementation
- `C:\Users\User\Documents\CodeFolder\scripts\draw-env.ps1` (reads `.env.template` and calls `op inject -i ... -o ... --force`)
- `C:\Users\User\Documents\CodeFolder\scripts\wipe-env.ps1` (deletes local `.env`)
- `%USERPROFILE%\.claude\settings.json` (agent `SessionStart`/`SessionEnd` hooks)
- `C:\Users\User\Documents\PowerShell\Microsoft.PowerShell_profile.ps1` (dynamic shell directory change hooks)
- `SkyHigh/.env.template` (contains standard 1Password links)

### Reversibility
**High.** Can revert to manual `.env` files by deleting the template and disable automatic hooks, or fully move to `direnv` with `.envrc` by shifting from `op inject` to `op read`.
